...
* テスト実行
    ...
    * 実行後のスクショ
        ...
        * ↑ではなく、8080ポートを使用していない為、エラー発生、組み込みサーバーでポート指定(8080)で起動し、再度テスト実行
          ```zsh
          $ rails s -b 0.0.0.0 -p 8080
          ```
          * 実行後のスクショ
          ![lecture11-3](images/lecture11-3.png)
...
* ↑解決した！絶対パスで記載したつもりだったが、最初の`/`が抜けていた為、接続できなかった。以下で接続できるようになった。
  * 修正.ssh/config
    ```.ssh/config
    Host "ホスト名"
        HostName ec2-44-203-229-70.compute-1.amazonaws.com
